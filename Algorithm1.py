...
def PINN(data, target, mask, config, library_function, library_config, train_opts, output_opts):
    ...
    with tf.Session() as sess:
        ...
        for iteration in np.arange(train_opts['max_iterations']):
            ...
            #print('1st fit')
            sess.run(train_op) 
            first_LR_loss = sess.run(graph.cost_PI) + sess.run(graph.cost_L1) + sess.run(graph.cost_MSE)
            first_mask = mask
            ...
            sess.run(train_op)
            second_LR_loss = sess.run(graph.cost_PI) + sess.run(graph.cost_L1) + sess.run(graph.cost_MSE)
            
            if second_LR_loss > first_LR_loss:
